...
export class WCDToolboxItemActionGroup extends WcdToolboxItemBase {
    ...
    firstUpdated() {
        ...
        const inGroup = this.elICA.getAttribute(icaGlobal.ATTRGROUP);
    ...
    private initClick(e: MouseEvent): void {
        ...
        if (!this.elMain || !this.elICA || !this.myParent || !document.defaultView) return;
        ...
        if (inGroup && inGroup === 'true') { 
            this.elICA.removeAttribute(icaGlobal.ATTRGROUP);
            this.innerHTML = unlock;
            return;
        } else {
            this.innerHTML = lock;
