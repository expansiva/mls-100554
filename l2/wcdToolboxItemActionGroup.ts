...
export class WCDToolboxItemActionGroup extends LitElement {
    ...
    firstUpdated() {
        ...
        const inGroup = this.elFCA.getAttribute('isfcagroup');
    ...
    private initClick(e: MouseEvent): void {
        ...
        if (inGroup && inGroup === 'true') { 
            this.elFCA.removeAttribute('isfcagroup');
            this.innerHTML = unlock;
            return;
        } else {
            this.innerHTML = lock;
            this.elFCA.setAttribute('isfcagroup', 'true');
        }
    }
}
...
export const getTemplate = (mode: string = '', position: string = ''): IActionsToolbox => {
    ...
    let ret: IActionsToolbox = templateActionGroup.group as IActionsToolbox;
