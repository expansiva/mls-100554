...
import { html, css, repeat, TemplateResult } from 'lit';
import { customElement, property, state } from 'lit/decorators.js';
import { StateLitElement } from './_100554_stateLitElement';
import { collab_file_pen, collab_bug_x12, collab_lightbulb, collab_trash, collab_plus, collab_triangle_exclamation, collab_rectangle_list } from './_100554_collabIcons';
...
export class PluginPreviewInsights100554 extends StateLitElement {
  ...
  renderi18nWarnings() {
    return html`
    <h1>i18nWarnings</h1>
    <widget-defs-list-edit-100554 .listItem=${this.defs?.codeInsights?.i18nWarnings}></widget-defs-list-edit-100554>
    `
  }
              ...
              <div class="user-story">
                <details>
                  ...
                  <span class="${totalDone === total ? 'summary-done' : 'summary-pending'}"> ${totalDone === total ? '✔' : collab_triangle_exclamation} Done (${totalDone}/${total})</span> 
                    -
                    <span @click=${ (e:MouseEvent) => e.preventDefault()} contenteditable>${story.story} </span>
                    <span @click=${ (e:MouseEvent) => this.removeStorie(e, storyIdx)}> ${collab_trash}</span>
                  </summary>
                  <div>
                    ${repeat(
        story.derivedRequirements,
        ((item: mls.l4.Planning, index: number) => item.description) as () => string,
        ((req: mls.l4.Planning, reqIdx: number) =>
          html`
