...
const message_pt = {
    ...
    selectPreReady: 'Selecione uma função pre programada.'
...
export class ScenarioInsertEventOrChange extends LitElement {
    ...
    private devicePR: string | undefined
    ...
    renderPreReady() {
        return html`
            ...
            <div class="scenarioHowDo">
                ...
                <div class="container">
                    ${repeat(this.preRotinas, ((key: any, idx: number) => key.event + idx) as any,
            ((item: any, index: any) => {
    ...
    renderItem(item: any, idx: number) {
        return html`
            <div class="card" .info=${item} @click="${this.selectedItemPR}">
                ${unsafeHTML((this.iconsEvents as any)[item.event as any])}
                <h2>${item.title}</h2>
                <p>${item.desc}</p>
            </div>
        `;
    ...
    private selectedItemPR(e: MouseEvent) {

        e.stopPropagation();

        let el = e.target as HTMLElement;
        if (el.className !== 'card') {
            el = el.closest('.card') as HTMLElement;
        }
        ...
        mls.events.fire(2, 'WidgetAction' as any, JSON.stringify(j));
    }
    ...
    private preRotinas: { event: string, title: string, desc: string, code: string }[] = [
