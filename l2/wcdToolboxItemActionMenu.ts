...
const templateActionMenuSub = {
    goToParents: {
        ...
        onclick: (e: MouseEvent, wc: WCDToolbox) => {
            ...
            const goToParent = (el: IcaPageOverlayItem) => {

                if (!el) return;
                let ica: IcaPageOverlayItem | HTMLElement = el;

                if (el.info && el.info.element) ica = el.info.element;
                let parent = ica.parentElement;
                if (!parent) parent = ica.getRootNode() ? (ica.getRootNode() as any).host : null;
                if (!parent) return;
                const tag = parent.tagName.toLowerCase();

                if (!tag.startsWith(`${icaGlobal.PREFIX}-`)) goToParent(parent as IcaPageOverlayItem);
            ...
            goToParent(wc.parentElement as IcaPageOverlayItem);
    ...
    goToFirstChild: {
        ...
        onclick: (e: MouseEvent, wc: WCDToolbox) => {
            ...
            const goToFirstChild = (element: IcaPageOverlayItem) => {

                if (!element.info || !element.info.element) return;
                const ica = element.info.element;
                if (!ica) return;

                if (ica.children.length === 0) return;
                ...
                const arrChildren = (Array.from(ica.shadowRoot ? ica.shadowRoot.children : ica.children));
            ...
            goToFirstChild(wc.parentElement as IcaPageOverlayItem);
    ...
    removeMe: {
        ...
        onclick: (e: MouseEvent, wc: WCDToolbox) => {
            const element = wc.parentElement as IcaPageOverlayItem;
            if (!element.info || !element.info.element) return;
            const ica = element.info.element;
            if (!ica) return;
            element.info.element.remove();
            element.remove();
