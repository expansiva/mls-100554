...
export class PreviewModeSinglePage {
    ...
    private async configIframe_old() {
        ...
        const storFileTest = mls.stor.getKeyToFile({ ...this.file, extension: '.test.ts' });
        const src =
            `/_${this.file.project}_/l2/` +
            `${this.file.folder ? this.file.folder + '/' : ''}` +
            `${this.file.shortName}.test.js`;
        if (mls.stor.files[storFileTest] && await this.checkIfExists(src)) {
            allImports.push(src);
        }
    ...
    private async checkIfExists(src: string) {
