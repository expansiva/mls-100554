...
export function canMoveElement( elementTag: string, newParentTag: string): boolean {

    elementTag = removeProjectNumberSegment(elementTag.toLocaleLowerCase());
    newParentTag = removeProjectNumberSegment(newParentTag.toLocaleLowerCase());

    const element = icaDescriptions[elementTag];
    const newParent = icaDescriptions[newParentTag];

    if (!element || !newParent) return false; 

    if (newParent.allowedChildren.includes("!*")) {
        return false;
    }
    
    const parentCanHaveChild = newParent.allowedChildren.some(childPattern => {
    ...
    if (!parentCanHaveChild) return false; 

    const childCanHaveParent = element.allowedParents.some(parentPattern => {
    ...
    if (!childCanHaveParent && element.allowedParents.length > 0) return false; 
